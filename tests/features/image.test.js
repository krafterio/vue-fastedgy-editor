...
function editorWithPictures(content, options = {}) {
...
describe('image', () => {
    it('lands a dropped picture where the pointer let go', async () => {
        const editor = editorWithPictures('<p>avant</p><p>après</p>');
    ...
    it('leaves a block moved inside the document alone', () => {
        const editor = editorWithPictures('<p>avant</p>');
    ...
    it('keeps an address the browser cannot fetch off `src`', () => {
        ...
        const html = serialised('<p></p><img data-src="attachment:15">');

        expect(html).toContain('data-src="attachment:15"');
        expect(html).not.toMatch(/\ssrc=/);
    });

    it('leaves an address it can fetch on `src`', () => {
        expect(serialised('<p></p><img src="https://melimelo.app/a.png">')).toContain(
